...
- `bash -O globstar -n .bash_profile welcome.sh bootstrap.sh scripts/**/*.sh`: syntax-check shell scripts before committing.
...
- `./bootstrap.sh --yes`: authenticate github.com before cloning the private `.ssh` submodule (honoring `GH_TOKEN`/`GITHUB_TOKEN`, otherwise installing `gh` into `~/.local` and running `gh auth login` when stdin and stdout are both terminals, skipped by `BOOTSTRAP_SKIP_GITHUB_AUTH=1` and inert unless the submodule resolves to an `https://github.com/` URL), link dotfiles into `$HOME`, initialize git submodules (the vendored tmux plugins under `.config/tmux/plugins/`), generate `$HOME/local.sh` through `tzselect` and prompt for `$HOME/.config/git/local` when absent, ensure nvm has the Node major pinned by the repository-local `.nvmrc` without changing nvm's default alias, and run `npm ci`; set `BOOTSTRAP_TZ`, `BOOTSTRAP_GIT_NAME`, `BOOTSTRAP_GIT_EMAIL`, and `GH_TOKEN` for an unattended first apply. `.nvmrc` itself is not linked into `$HOME`, existing local settings are preserved, and existing link targets are backed up under `$HOME/.dotfiles-backup/`.
